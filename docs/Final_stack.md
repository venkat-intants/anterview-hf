...
| **Embeddings + vector store**     | **`gemini-embedding-001`** (Google Gemini, 3072-dim, free tier) stored in **pgvector** (`halfvec`) | Same key and provider as the LLM row above, so one fewer credential to manage. This row previously named OpenAI `text-embedding-3-large`; the code has used Gemini since migration `a9c1e2f3b4d5` (2026-06-27) for resume search, and PH5-E2 reuses the same call for the document corpus. Not India-region — see `docs/DATA-FLOW.md`. pgvector avoids a separate Qdrant/Weaviate cluster = one less thing to break in 15 days.                                                                                                                                          |
