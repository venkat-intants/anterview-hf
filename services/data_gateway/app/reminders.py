...
enqueue, so the windows can overlap and the sweep can run as often as it likes
without a candidate receiving the same reminder twice. The key names the thing
and the window — ``exam_reminder:<assignment_id>:24h`` — never the time it fired.
...
# Reminder windows, longest first. A candidate invited with less than a day's
# notice skips the 24h reminder and still gets the 1h one, because each window
# is evaluated independently rather than as a sequence.
_WINDOWS: tuple[tuple[str, timedelta], ...] = (
...
async def _exam_reminders(db: AsyncSession, result: SweepResult) -> None:
    ...
    for label, delta in _WINDOWS:
        rows = (
            await db.execute(
                text(_EXAM_DUE_SQL),
                {"now": now, "horizon": now + delta, "lim": _BATCH},
            )
        ).mappings().all()
        for r in rows:
...
SELECT inv.id, inv.expires_at, inv.scheduled_at, inv.language, inv.company_id,
 ...
 WHERE inv.status = 'invited'
   ...
   AND a.email IS NOT NULL
   AND COALESCE(inv.scheduled_at, inv.expires_at) > :now
    ...
    for label, delta in _WINDOWS:
        ...
        for r in rows:
            sent = await enqueue_email(
                ...
                related_id=r["id"],
                dedupe_key=f"interview_reminder:{r['id']}:{label}",
            )
            if sent is not None:
...
# ---------------------------------------------------------------------------
_LAPSED_SQL = """
SELECT 'exam' AS kind, asg.id, asg.expires_at, asg.company_id, asg.created_by_user_id,
...
UNION ALL
SELECT 'interview' AS kind, inv.id, inv.expires_at, inv.company_id, inv.created_by_user_id,
...
async def _expiry_notices(db: AsyncSession, result: SweepResult) -> None:
    ...
    for r in rows:
        key = f"expiry_notice:{r['kind']}:{r['id']}"
        sent = await enqueue_email(
            db,
            to=r["email"],
        ...
        if sent is None:
            continue  # already notified for this link
