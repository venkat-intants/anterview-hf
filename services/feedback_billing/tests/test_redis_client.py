"""SH-2: feedback_billing's Redis client must carry the shared Upstash hardening.
...
service's client is the hardened one — feedback_billing used to build its own
pool with nothing but decode_responses/max_connections, which is how it
inherited the intermittent "connection forcibly closed" 500s that data_gateway
had already fixed. No I/O happens: redis-py pools are lazy.
"""
